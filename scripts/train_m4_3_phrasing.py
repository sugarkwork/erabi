"""Training script for M4.3-P Priority Phrasing Diversification.

Executes a single standard Cross-Entropy fine-tuning run from B0 on 1080 combined cases:
- 600 M3 base train cases (data/m3_3_v2/train.jsonl)
- 240 M4.1 exception train cases (data/m4_1_exception/train_exception.jsonl)
- 240 M4.3 phrasing train cases (data/m4_3_phrasing/phrasing_train.jsonl)

Dev evaluation on 220 cases:
- 100 M3 base dev cases
- 40 M4.1 exception dev cases
- 80 M4.3 phrasing dev cases

Checkpoint Selection Criteria (Section 13):
1. M3 dev accuracy >= 95%
2. M4.1 exception dev accuracy >= 95%
3. Maximum M4.3 phrasing dev diff-target pair both
4. Tie-break: lowest phrasing dev NLL

Saves best checkpoint to runs/m4_3_phrasing/trained/checkpoint.
...
logger = logging.getLogger("erabi.train_m4_3p")

OUT_DIR = ROOT / "runs/m4_3_phrasing/trained"
OUT_DIR.mkdir(parents=True, exist_ok=True)

TRAIN_FILE = ROOT / "data/m4_3_phrasing/combined_train.jsonl"
DEV_FILE = ROOT / "data/m4_3_phrasing/combined_dev.jsonl"
...
def evaluate_dev(trainer: Trainer, dev_records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # 3. M4.3 phrasing dev subset (80 cases / 40 pairs)
    phr_results = [r for r in results if r.get("task_family") == "phrasing_diversification"]
