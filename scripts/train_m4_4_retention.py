"""Training script for M4.4-R Targeted Retention Replay (W_m4_4r).

Executes a single standard Cross-Entropy fine-tuning run from B0 on 1184 combined cases:
...
- 104 targeted retention replay cases (48 R1 equality boundary + 56 R2 4-state balanced composite logic)

Dev evaluation on 220 cases:
- 100 M3 base dev cases (with diagnostic display of R1 and R2 subsets)
- 40 M4.1 exception dev cases
- 80 M4.3.1 phrasing dev cases

Checkpoint Selection Criteria (maintained from M4.3.1):
...
Saves best checkpoint to runs/m4_4_retention/trained/checkpoint.
...
logger = logging.getLogger("erabi.train_m4_4r")

OUT_DIR = ROOT / "runs/m4_4_retention/trained"
OUT_DIR.mkdir(parents=True, exist_ok=True)

TRAIN_FILE = ROOT / "data/m4_4_retention/combined_train_retention.jsonl"
...
def evaluate_dev(trainer: Trainer, dev_records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Diagnostic: R1 and R2 subsets on M3 dev
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        logger.info(
            ...
            f"M3 Acc: {dev_m['m3_accuracy']*100:.1f}% (R1: {dev_m['diagnostic_r1_dev_accuracy']*100:.0f}%, R2: {dev_m['diagnostic_r2_dev_accuracy']*100:.0f}%) | "
