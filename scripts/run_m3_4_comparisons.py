"""Standardized comparison script for ERABI M3.4 (W_v2_gradfix).

Evaluates W_v2_gradfix on:
...
Reuses saved predictions from runs/m3_3_v2/comparisons/ for B0, W_fix, W_v2.
Computes:
...
- smoke 12 cases per-case transitions
- 178-case non-novel-semantic-overlap auxiliary metric
- Saves outputs to runs/m3_4_gradfix/comparisons/ and config.json
"""
...
def main():
    out_dir = ROOT_DIR / "runs" / "m3_4_gradfix" / "comparisons"
    out_dir.mkdir(parents=True, exist_ok=True)

    # 1. Config and file hashes
    config = {
        "model_name": "W_v2_gradfix",
        ...
        "fix_description": "Micro-batch loss weighted by (len(batch_records) / window_sample_count) to exactly match full-window mean gradient.",
    }
    with open(ROOT_DIR / "runs" / "m3_4_gradfix" / "config.json", "w", encoding="utf-8") as f:
        json.dump(config, f, indent=2, ensure_ascii=False)

    # 2. Run inference for W_v2_gradfix
    gradfix_ckpt = ROOT_DIR / "runs" / "m3_4_gradfix" / "trained" / "checkpoint"
    print(f"Loading W_v2_gradfix from {gradfix_ckpt}...")
    engine = GLiClassEngine(model_id=str(gradfix_ckpt))
    ...
    for ds_name, ds_path in datasets:
        print(f"Evaluating W_v2_gradfix on {ds_name}...")
        res = evaluate_engine_on_file(engine, ds_path)
        new_results[ds_name] = res
        pred_file = out_dir / f"W_v2_gradfix_{ds_name}_predictions.jsonl"
        with open(pred_file, "w", encoding="utf-8") as f:
            for p in res["predictions"]:
                f.write(json.dumps(p, ensure_ascii=False) + "\n")

    # 3. Load saved predictions for B0, W_fix, W_v2
    all_preds = {
        ...
        "W_v2_gradfix": {ds: {p["id"]: p for p in new_results[ds]["predictions"]} for ds, _ in datasets},
    }

    # 4. Detailed analysis on eval_v2 (200 cases / 100 pairs)
    eval_records = load_jsonl(ROOT_DIR / "data" / "m3_3_v2" / "eval_v2.jsonl")
    groups = defaultdict(list)
    for r in eval_records:
        groups[r["group_id"]].append(r)

    models = ["B0", "W_fix", "W_v2", "W_v2_gradfix"]

    # Identify semantic overlap cases (11 pairs / 22 cases in novel composite_logic and boundary sharing state with train/dev)
    train_records = load_jsonl(ROOT_DIR / "data" / "m3_3_v2" / "train.jsonl")
    dev_records = load_jsonl(ROOT_DIR / "data" / "m3_3_v2" / "dev.jsonl")

    # Exact 11 pairs (22 cases) of novel explicit_rule sharing state with train/dev
    overlap_gids = {
    ...
    for cat, is_diff_target in [("diff_target_47", True), ("same_target_53", False)]:
        ...
        for m in models:
            ...
            if is_diff_target:
                ...
                entry["pred_same_failure"] = pred_same if both_correct == 0 else (pred_same - (both_correct if False else 0))  # in diff_target, both_correct cannot have pred_same!
                entry["pred_same_failure"] = pred_same  # because if pred_same, at least one is wrong
    ...
    # Smoke cases individual breakdown
    ...
    print("\n=======================================================")
    print("=== ERABI M3.4 COMPARISON SUMMARY (B0, W_fix, W_v2, W_v2_gradfix) ===")
    print("=======================================================")
    ...
    for m in models:
        ...
        print(f"  {m:14s} | Both: {d['both_correct']:2d}/47 ({d['both_correct']/47*100:4.1f}%) | Pred Same (未切替): {d['pred_same']:2d} | Pred Diff (切替): {d['pred_diff']:2d}")
