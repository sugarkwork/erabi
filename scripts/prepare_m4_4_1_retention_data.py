"""M4.4.1 Targeted Retention Replay Data Preparation Script for ERABI.
...
2. R1b (Equality Comparison): inventory == demand, 4 groups = 8 cases
   - train-er-0031: 38 == 38
   - train-er-0151: 55 == 55
   - train-er-0199: 59 == 59
   - train-er-0283: 40 == 40
3. R2 (Composite Logic): 4-state balanced (HP < thresh, item_has), 7 groups each = 28 groups = 56 cases
Total replay: 56 groups = 112 cases (<= 160 case cap).

Builds combined_train_retention.jsonl:
- 600 M3 base train
- 240 M4.1 exception train
- 240 M4.3.1 phrasing train
- 112 retention replay
Total = 1192 cases (<= 1240 cap).

Outputs:
- data/m4_4_1_retention/combined_train_retention.jsonl
- data/m4_4_1_retention/retention_manifest.json
- runs/m4_4_1_retention/retention_source_audit.json
"""
...
OUT_DATA_DIR = ROOT / "data/m4_4_1_retention"
OUT_RUN_DIR = ROOT / "runs/m4_4_1_retention"
...
def extract_retention_candidates() -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    # 2. R1b: Equality Comparison (inventory == demand)
    EXPECTED_R1B_GIDS = {"train-er-0031", "train-er-0151", "train-er-0199", "train-er-0283"}
    comparison_groups: Dict[str, List[Dict[str, Any]]] = {}
    for r in m3_records:
        if r.get("rule_kind") == "comparison":
            comparison_groups.setdefault(r["group_id"], []).append(r)

    r1b_groups: List[Tuple[str, List[Dict[str, Any]]]] = []
    for gid, pair in sorted(comparison_groups.items()):
        if len(pair) == 2:
            ctx = pair[0]["context"]
            nums = [int(x) for x in re.findall(r"\d+", ctx)]
            if len(nums) >= 2 and nums[0] == nums[1]:
                r1b_groups.append((gid, pair))

    found_r1b_gids = {gid for gid, _ in r1b_groups}
    assert found_r1b_gids == EXPECTED_R1B_GIDS, f"R1b groups mismatch: found {found_r1b_gids}, expected {EXPECTED_R1B_GIDS}"

    r1b_records = [r for gid, pair in r1b_groups for r in pair]

    # 3. R2: Composite Logic (4-state balanced)
    ...
    # Balance 4 states: take 7 groups from each state (same as M4.4)
    TARGET_PER_STATE = 7
    ...
    all_replay_records = r1_records + r1b_records + r2_records
    ...
    audit_info = {
        ...
        "r1b_equality_comparison": {
            "description": "inventory == demand comparison pairs",
            "total_groups": len(r1b_groups),
            "total_cases": len(r1b_records),
            "group_ids": [gid for gid, _ in r1b_groups],
        },
        "r2_composite_logic": {
        ...
        "total_replay_groups": len(r1_groups) + len(r1b_groups) + len(r2_groups),
...
def main():
    print("=== ERABI M4.4.1 Retention Replay Data Preparation ===")
    ...
    print(f"Extracted R1b (Equality Comparison): {audit_info['r1b_equality_comparison']['total_cases']} cases ({audit_info['r1b_equality_comparison']['total_groups']} groups)")
    ...
    manifest = {
        "dataset_name": "m4_4_1_retention_combined_train",
        "description": "Combined training set with targeted M3 retention replay including R1b equality comparison",
        "base_components": {
