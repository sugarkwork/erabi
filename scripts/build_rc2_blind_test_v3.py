"""Build Blind Sealed Acceptance Test Suite v3 for ERABI Release Candidate 2.
...
3. 8 balanced families (60 cases / 30 pairs each):
   - core_rules (30 pairs, prefix rc2b3_core_)
   - logical_operators (30 pairs, prefix rc2b3_op_)
   - priority_exception (30 pairs, prefix rc2b3_prio_)
   - natural_japanese (30 pairs, prefix rc2b3_nat_)
   - domain_transfer (30 pairs, prefix rc2b3_dom_)
   - general_choice (30 pairs, prefix rc2b3_gen_)
   - variable_choice (30 pairs, prefix rc2b3_var_)
   - perturbation_invariance (30 pairs, prefix rc2b3_pert_)
...
7. Rigorous zero-leakage audit against all background training, dev, calibration records.
...
from blind_v3_data.family_core_rules import get_core_rules_pairs
from blind_v3_data.family_domain_transfer import get_domain_transfer_pairs
from blind_v3_data.family_general_choice import get_general_choice_pairs
from blind_v3_data.family_logical_operators import get_logical_operators_pairs
from blind_v3_data.family_natural_japanese import get_natural_japanese_pairs
from blind_v3_data.family_perturbation_invariance import get_perturbation_invariance_pairs
from blind_v3_data.family_priority_exception import get_priority_exception_pairs
from blind_v3_data.family_variable_choice import get_variable_choice_pairs

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("erabi.build_rc2_blind_v3")

OUT_DIR = ROOT / "data/sealed_acceptance_rc2_blind_v3"
OUT_PATH = OUT_DIR / "sealed_test_rc2_blind_v3.jsonl"
...
def run_token_contract_audit(cases: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for idx, c in enumerate(cases):
        ...
        if total_tokens > 512:
            ...
            audit_warning = f"Case {c['id']} is close to limit ({total_tokens} > 450)"
...
def run_zero_leakage_audit(cases: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    check_paths = [
        p for p in (ROOT / "data").glob("**/*.jsonl") if "sealed_acceptance_rc2_blind" not in str(p)
...
def main():
    logger.info("=== Starting Blind Test Suite v3 Generation (Phase A: Zero Model Inference) ===")
    ...
    logger.info(f"Wrote blind sealed test suite v3 to {OUT_PATH}")
    ...
    # 6. Manifest
    manifest = {
        "suite_name": "data/sealed_acceptance_rc2_blind_v3",
        "filename": "sealed_test_rc2_blind_v3.jsonl",
    ...
    logger.info("=== Phase A: Test Authoring Blind v3 Completed Successfully ===")
