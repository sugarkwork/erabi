"""Milestone 6 Experiment 2: Diversified Operator Training Script (ERABI).

Roadmap Reference:
Milestone 6 Operator Generalization (Section 7).
Budget: Experiment 2 of 3 for Milestone 6.

Pre-registered Hypothesis:
Experiment 1 demonstrated that while 8 of 10 operator families learned successfully,
boundary inequality operators (ge_vs_gt, le_vs_lt) and negation suffered from single-template
phrasing overfit in training. By training on train_operator.jsonl with 4 distinct phrasing variants
per operator (400 records total), the model will learn the invariant semantics of inclusive/exclusive
bounds and negation across diverse linguistic expressions, driving fresh_operator_eval accuracy >= 85.0%
and pair both >= 70.0%, while maintaining Core retention (eval_v2 >= 96.0%, eval_exception >= 98.0%).

Data streams:
- Stream A (Core Logic & Retention): 712 records (600 M3 + 112 Replay)
- Stream B (Multitask Operators & Priority): 880 records (240 M4.1 + 240 M4.3.1 + 400 M6 Diversified Train)
Every micro-batch: [1 from Stream A, 1 from Stream B].

Outputs:
- runs/m6_e2_diversified/checkpoints/
- runs/m6_e2_diversified/evaluation_report.json
- runs/m6_e2_diversified/notes.md
"""
...
logger = logging.getLogger("erabi.m6_e2_diversified")

OUT_DIR = ROOT / "runs/m6_e2_diversified"
...
def load_data_streams() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    # Stream B: Multitask Priority & Operators (240 + 240 + 400 = 880 records)
    ...
    assert len(stream_b) == 880, f"Expected 880 in Stream B, got {len(stream_b)}"
...
def main():
    ...
    logger.info("=== ERABI Milestone 6 Experiment 2: Diversified Operator Training ===")
    ...
    for epoch in range(1, epochs + 1):
        ...
        n_max = max(len(shuffled_a), len(shuffled_b))  # 880
        micro_batches = []
    ...
    # Save summary report
    report = {
        "experiment_name": "Milestone 6 Experiment 2: Diversified Operator Training",
    ...
    md_lines = [
        "# Milestone 6 Experiment 2: Diversified Operator Report\n",
