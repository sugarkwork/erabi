"""Build Blind Sealed Acceptance Test Suite v2 for ERABI Release Candidate 2.

Directive Reference:
ERABI_RC2_BLIND_SEALED_REACCEPTANCE_DIRECTIVE.md (Milestone 24.1).
...
3. 8 balanced families (60 cases / 30 pairs each):
   - core_rules (30 pairs)
   - logical_operators (30 pairs)
   - priority_exception (30 pairs)
   - natural_japanese (30 pairs)
   - domain_transfer (30 pairs)
   - general_choice (30 pairs)
   - variable_choice (30 pairs)
   - perturbation_invariance (30 pairs)
...
5. Structured distractor taxonomy: plausible, close-call, irrelevant, standby.
6. Rigorous zero-leakage audit against all background records.
...
from blind_v2_data.family_core_rules import get_core_rules_pairs
from blind_v2_data.family_domain_transfer import get_domain_transfer_pairs
from blind_v2_data.family_general_choice import get_general_choice_pairs
from blind_v2_data.family_logical_operators import get_logical_operators_pairs
from blind_v2_data.family_natural_japanese import get_natural_japanese_pairs
from blind_v2_data.family_perturbation_invariance import get_perturbation_invariance_pairs
from blind_v2_data.family_priority_exception import get_priority_exception_pairs
from blind_v2_data.family_variable_choice import get_variable_choice_pairs

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("erabi.build_rc2_blind_v2")

OUT_DIR = ROOT / "data/sealed_acceptance_rc2_blind_v2"
OUT_PATH = OUT_DIR / "sealed_test_rc2_blind_v2.jsonl"
...
def run_zero_leakage_audit(cases: List[Dict[str, Any]]) -> Dict[str, Any]:
    logger.info("Verifying ZERO data leakage against all training, calibration, and past evaluation sets...")
    check_paths = [
        p for p in (ROOT / "data").glob("**/*.jsonl") if "sealed_acceptance_rc2_blind_v2" not in str(p)
    ...
    if not is_leak_free:
        ...
        logger.info("ZERO data leakage confirmed: 0 matches against all 48,804 background records.")

    return audit_result


def main():
    logger.info("=== Starting Blind Test Suite v2 Generation (Phase A: Zero Model Inference) ===")
    ...
    logger.info(f"Wrote blind sealed test suite to {OUT_PATH}")
    ...
    # 5. Manifest
    manifest = {
        "suite_name": "data/sealed_acceptance_rc2_blind_v2",
        "filename": "sealed_test_rc2_blind_v2.jsonl",
    ...
    logger.info("=== Phase A: Test Authoring Completed Successfully ===")
