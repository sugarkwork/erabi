"""Milestone 7 Experiment 1: Robustness Training Script (ERABI).

Roadmap Reference:
Milestone 7 Domain & Perturbation Robustness (Section 8).
Budget: Experiment 1 of 3 for Milestone 7.

Pre-registered Hypothesis:
Baseline evaluation of W_operator_v1 achieved 89.8% overall accuracy on fresh_robustness_eval,
but failed the domain gate on medical (58.3% < 60.0%) and smart_home (50.0% < 60.0%).
Training with 1:1 Task-Balanced Micro-Batches:
- Stream A: Core Logic & Replay (712 records: 600 M3 + 112 Replay)
- Stream B: Multitask Extensions (1,204 records: 240 M4.1 + 240 M4.3.1 + 400 M6 Op + 324 M7 Robustness)
will enable the model to generalize across all 9 domains (every domain >= 60%) and 6 perturbations,
reaching >= 85.0% accuracy and >= 95.0% permutation consistency on fresh_robustness_eval,
while preserving Core retention (eval_v2 >= 96.0%, eval_exception >= 98.0%, fresh_phrasing >= 78.0%)
and Operator retention (fresh_operator_eval >= 92.0%).

Outputs:
- runs/m7_e1_robustness/checkpoints/
- runs/m7_e1_robustness/evaluation_report.json
- runs/m7_e1_robustness/notes.md
"""
...
logger = logging.getLogger("erabi.m7_e1_robustness")

OUT_DIR = ROOT / "runs/m7_e1_robustness"
...
BENCHMARKS = {
    ...
    "dev_robustness": ROOT / "data/m7_robustness/dev_robustness.jsonl",
...
def load_data_streams() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    # Stream B: Multitask Priority, Phrasing, Operators & Robustness (240 + 240 + 400 + 324 = 1,204 records)
    ...
    stream_b = m4_1_train + m4_3_1_train + m6_op_train + m7_rob_train
    assert len(stream_b) == 1204, f"Expected 1204 in Stream B, got {len(stream_b)}"

    return stream_a, stream_b


def evaluate_dataset_with_engine(engine: GLiClassEngine, dataset_path: Path, check_permutation: bool = False) -> Dict[str, Any]:
    ...
    for row in records:
        ...
        if check_permutation:
            ...
            rev_req = ChoiceRequest.from_dict(rev_row)
    ...
    if check_permutation:
        metrics["perm_consistency"] = perm_consistent_count / n
    ...
    domains = sorted(list({r.get("domain") for r in results if r.get("domain")}))
    if len(domains) > 1:
        dom_m = {}
        for d in domains:
            d_rows = [r for r in results if r.get("domain") == d]
            d_corr = sum(1 for r in d_rows if r["is_correct"])
            dom_m[d] = {
                "count": len(d_rows),
                "correct": d_corr,
                "accuracy": d_corr / len(d_rows),
            }
        metrics["by_domain"] = dom_m

    # Perturbation breakdown if available
    perts = sorted(list({r.get("perturbation") for r in results if r.get("perturbation")}))
    if len(perts) > 1:
        pert_m = {}
        for p in perts:
            p_rows = [r for r in results if r.get("perturbation") == p]
            p_corr = sum(1 for r in p_rows if r["is_correct"])
            pert_m[p] = {
                "count": len(p_rows),
                "correct": p_corr,
                "accuracy": p_corr / len(p_rows),
            }
        metrics["by_perturbation"] = pert_m

    return metrics


def check_m7_gate(bench_results: Dict[str, Dict[str, Any]]) -> Tuple[bool, List[str]]:
    passed = True
    reasons = []

    # 1. fresh_robustness_eval
    frob = bench_results.get("fresh_robustness_eval", {})
    frob_acc = frob.get("accuracy", 0.0)
    frob_perm = frob.get("perm_consistency", 0.0)

    if frob_acc < 0.85:
        passed = False
        reasons.append(f"fresh robustness accuracy {frob_acc*100:.1f}% < 85.0%")
    if frob_perm < 0.95:
        passed = False
        reasons.append(f"fresh robustness perm consistency {frob_perm*100:.1f}% < 95.0%")

    # Check each domain >= 60.0%
    dom_dict = frob.get("by_domain", {})
    for d, dm in dom_dict.items():
        if dm["accuracy"] < 0.60:
            passed = False
            reasons.append(f"domain '{d}' acc {dm['accuracy']*100:.1f}% < 60.0%")
        if dm["accuracy"] == 0.0:
            passed = False
            reasons.append(f"domain '{d}' has 0.0% accuracy")

    # 2. Operator retention: fresh_operator_eval >= 92.0% (M6 was 94.0%)
    fop = bench_results.get("fresh_operator_eval", {})
    fop_acc = fop.get("accuracy", 0.0)
    if fop_acc < 0.92:
        passed = False
        reasons.append(f"fresh operator retention accuracy {fop_acc*100:.1f}% < 92.0%")

    # 3. Core retention: within -2pt of Milestone 5/6
    ...
    if fr_acc < 0.780:
        passed = False
        reasons.append(f"fresh phrasing retention accuracy {fr_acc*100:.1f}% < 78.0%")
...
def main():
    ...
    logger.info("=== ERABI Milestone 7 Experiment 1: Robustness Training ===")
    ...
    for epoch in range(1, epochs + 1):
        ...
        n_max = max(len(shuffled_a), len(shuffled_b))  # 1,204
        micro_batches = []
    ...
    logger.info("\n=== Evaluating Saved Checkpoints on Full Milestone 7 Suite ===")
    eval_results = {}
    best_m7_epoch = None
    best_m7_passed = False
    best_score = -1.0
    ...
    for ep in SAVE_EPOCHS:
        ...
        for bname, bpath in BENCHMARKS.items():
            check_perm = (bname == "fresh_robustness_eval")
            met = evaluate_dataset_with_engine(engine, bpath, check_permutation=check_perm)
            ep_metrics[bname] = met
            acc_str = f"Acc: {met['accuracy']*100:.1f}%"
            perm_str = f"Perm: {met['perm_consistency']*100:.1f}%" if "perm_consistency" in met else ""
            diff_str = f"Diff Both: {met.get('diff_both', 0)}/{met.get('diff_pairs', 0)} ({met.get('diff_both_rate', 0)*100:.1f}%)" if "diff_both" in met else ""
            logger.info(f"  {bname:22s} | {acc_str:14s} | {perm_str:12s} | {diff_str}")

        del engine
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

        is_m7_pass, gate_reasons = check_m7_gate(ep_metrics)
        logger.info(f"Epoch {ep} M7 Gate Pass: {is_m7_pass} | Reasons: {gate_reasons if not is_m7_pass else 'ALL PASSED'}")

        # Score formula: fresh_rob_acc + fresh_rob_perm + fop_acc + ev2_acc
        frob_acc = ep_metrics["fresh_robustness_eval"]["accuracy"]
        frob_perm = ep_metrics["fresh_robustness_eval"]["perm_consistency"]
        fop_acc = ep_metrics["fresh_operator_eval"]["accuracy"]
        ev2_acc = ep_metrics["eval_v2"]["accuracy"]
        score = frob_acc + frob_perm + fop_acc + ev2_acc

        eval_results[ep] = {
            "is_m7_pass": is_m7_pass,
            "gate_reasons": gate_reasons,
            "score": score,
            "metrics": ep_metrics,
        }

        if is_m7_pass and score > best_score:
            best_score = score
            best_m7_epoch = ep
            best_m7_passed = True
    ...
    # Process outcome
    if best_m7_passed and best_m7_epoch is not None:
        logger.info(f"\n=======================================================")
        logger.info(f"MILESTONE 7 GATE PASSED at Epoch {best_m7_epoch}! (Score: {best_score:.4f})")
        logger.info(f"=======================================================")

        best_ckpt = saved_paths[best_m7_epoch]
        target_dir = ROOT / "runs/m7_robustness/checkpoint"
        ...
        logger.info(f"Successfully frozen best checkpoint to {target_dir} as W_robustness_v1")

        # Manifest
        manifest = {
            "milestone": "Milestone 7 Domain & Perturbation Robustness",
            "model_version": "W_robustness_v1",
            "best_epoch": best_m7_epoch,
            "evaluation_report": eval_results[best_m7_epoch],
            "benchmark_summary": {
                "fresh_robustness_eval_accuracy": eval_results[best_m7_epoch]["metrics"]["fresh_robustness_eval"]["accuracy"],
                "fresh_robustness_eval_perm_consistency": eval_results[best_m7_epoch]["metrics"]["fresh_robustness_eval"]["perm_consistency"],
                "fresh_operator_eval_accuracy": eval_results[best_m7_epoch]["metrics"]["fresh_operator_eval"]["accuracy"],
                "eval_v2_accuracy": eval_results[best_m7_epoch]["metrics"]["eval_v2"]["accuracy"],
                "eval_exception_accuracy": eval_results[best_m7_epoch]["metrics"]["eval_exception"]["accuracy"],
                "fresh_phrasing_eval_accuracy": eval_results[best_m7_epoch]["metrics"]["fresh_phrasing_eval"]["accuracy"],
                "smoke_cases_correct": eval_results[best_m7_epoch]["metrics"]["smoke_cases"]["correct_count"],
            },
            "frozen_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        }
        manifest_path = ROOT / "runs/m7_robustness/manifest.json"
        with open(manifest_path, "w", encoding="utf-8") as f:
            json.dump(manifest, f, indent=2, ensure_ascii=False)

        # Create review bundle zip
        bundle_path = ROOT / "runs/m7_robustness/review_bundle.zip"
        with zipfile.ZipFile(bundle_path, "w", zipfile.ZIP_DEFLATED) as zf:
            zf.write(manifest_path, arcname="manifest.json")
            zf.write(report_path, arcname="evaluation_report.json")
            zf.write(ROOT / "data/m7_robustness/fresh_robustness_eval.jsonl", arcname="fresh_robustness_eval.jsonl")
            zf.write(ROOT / "runs/m7_baseline_diagnostic/baseline_report.json", arcname="baseline_report.json")
            zf.write(ROOT / "runs/m7_baseline_diagnostic/notes.md", arcname="baseline_notes.md")
        ...
        logger.error("No checkpoint passed the Milestone 7 Gate in Experiment 1.")
